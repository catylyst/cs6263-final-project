...
from __future__ import (
    annotations,  # Delays type-hint evaluation, which helps with cleaner modern type hints.
)
...
from sklearn.feature_extraction.text import (
    TfidfVectorizer,  # Converts text documents into TF-IDF numeric vectors.
)
from sklearn.metrics.pairwise import (
    cosine_similarity,  # Measures similarity between the query vector and document vectors.
)

from myproject.schemas import (
    CorpusDocument,  # Imports the Pydantic model used to validate corpus documents.
)
...
class Retriever:
    ...
    def __init__(self, corpus_path: str) -> None:
        ...
        self.corpus_path = Path(
            corpus_path
        )  # Converts the input file path string into a Path object.

        self.documents = self._load_corpus(
            self.corpus_path
        )  # Loads and validates all documents from the JSONL corpus.

        if not self.documents:  # Checks whether the corpus file loaded zero valid documents.
            raise ValueError(
                f"No documents found in corpus: {corpus_path}"
            )  # Stops execution if the corpus is empty.

        self.texts = [
            doc.text for doc in self.documents
        ]  # Extracts only the text field from each corpus document.

        self.vectorizer = TfidfVectorizer(
            stop_words="english"
        )  # Creates a TF-IDF vectorizer and removes common English stop words.

        self.matrix = self.vectorizer.fit_transform(
            self.texts
        )  # Learns the vocabulary and converts all corpus texts into vectors.
    ...
    def _load_corpus(self, corpus_path: Path) -> list[CorpusDocument]:
        ...
        if not corpus_path.exists():  # Checks whether the JSONL corpus file exists.
            ...
            )  # Stops execution if the file path is invalid.

        documents: list[CorpusDocument] = (
            []
        )  # Creates an empty list to store validated corpus documents.

        with corpus_path.open(
            "r", encoding="utf-8"
        ) as file:  # Opens the corpus file for reading using UTF-8 encoding.
            for line_number, line in enumerate(
                file, start=1
            ):  # Loops through the file one line at a time and tracks line numbers.
                stripped = (
                    line.strip()
                )  # Removes leading and trailing whitespace from the current line.
                ...
                try:  # Starts a block for code that might fail while parsing JSON.
                    row = json.loads(
                        stripped
                    )  # Converts the JSON string from the current line into a Python dictionary.

                    documents.append(
                        CorpusDocument(**row)
                    )  # Validates the dictionary using CorpusDocument and adds it to the list.
    ...
    def search(self, query: str, k: int = 5) -> list[dict]:
        ...
        cleaned_query = " ".join(
            query.strip().split()
        )  # Cleans the query by removing extra spaces, tabs, and newlines.
        ...
        query_vector = self.vectorizer.transform(
            [cleaned_query]
        )  # Converts the user query into a TF-IDF vector using the same vocabulary.

        similarities = cosine_similarity(
            query_vector, self.matrix
        ).flatten()  # Compares the query vector to every document vector.

        ranked_indices = similarities.argsort()[
            ::-1
        ]  # Sorts document indexes from highest similarity score to lowest.
        ...
        for index in ranked_indices[:k]:  # Loops through only the top k ranked document indexes.
            score = float(
                similarities[index]
            )  # Gets the similarity score for the current document as a regular float.
            ...
            doc = self.documents[
                index
            ]  # Retrieves the original CorpusDocument that matches the ranked index.
            ...
            results.append(  # Adds a formatted citation-style result to the output list.
                {
                    ...
                    "snippet": self._make_snippet(
                        doc.text
                    ),  # Includes a shortened text snippet from the document.
                    "score": round(
                        score, 4
                    ),  # Includes the similarity score rounded to four decimal places.
                }
            )
    ...
    def _make_snippet(text: str, max_chars: int = 280) -> str:
        ...
        clean_text = " ".join(
            text.strip().split()
        )  # Cleans the document text by removing extra whitespace.
        ...
        return (
            clean_text[: max_chars - 3].rstrip() + "..."
        )  # Truncates long text and adds an ellipsis.
