...
def _normalize(text: str) -> str:
    ...
    )  # This function takes a string input, converts it to lowercase, removes leading and trailing whitespace, and collapses multiple spaces into a single space. This normalization helps ensure that the pattern matching is consistent regardless of the input formatting.
...
def _count_question_marks(text: str) -> int:
    ...
    return text.count(
        "?"
    )  # This function counts the number of question marks in the input text, which can be an indicator of question complexity.
...
def _contains_any(text: str, phrases: list[str]) -> bool:
    ...
    )  # This function checks if any of the specified phrases are present in the input text, which can be used to identify certain types of questions that may require retrieval.
...
def classify_query(text: str) -> str:
    ...
    normalized = _normalize(
        text
    )  # The input text is normalized to ensure consistent pattern matching.

    if not normalized:
        raise ValueError(
            "input text is required"
        )  # If the normalized text is empty, a ValueError is raised indicating that input text is required for classification.
    ...
    word_count = len(
        re.findall(r"\b\w+\b", normalized)
    )  # This line uses a regular expression to find all word tokens in the normalized text and counts them. The pattern \b\w+\b matches sequences of word characters that are bounded by word boundaries, effectively counting the number of words in the question.
    question_count = _count_question_marks(
        normalized
    )  # This line counts the number of question marks in the normalized text, which can be an indicator of question complexity. A higher number of question marks may suggest that the question is more complex and may require more retrieval steps to answer.
