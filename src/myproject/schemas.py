...
class Citation(BaseModel):
    ...
    score: float = Field(
        ge=0.0
    )  # The relevance score of the retrieved passage, which can be useful for debugging and analysis.
...
class QueryResponse(BaseModel):
    ...
    citations: list[
        Citation
    ]  # A list of retrieved evidence passages that were used to generate the answer, which can be useful for transparency and debugging.
    retrieval_steps: int = Field(
        ge=0
    )  # The number of retrieval steps taken to generate the answer, which can be useful for analysis and debugging.
    latency_ms: int = Field(
        ge=0
    )  # The total latency of the request in milliseconds, which can be useful for performance monitoring and debugging.
